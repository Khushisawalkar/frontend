    ...
    <main className="relative min-h-screen overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 mt-20">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#FFC801] via-[#FF9932] to-[#114C5A] shadow-[0_0_40px_rgba(255,200,1,0.3)] flex items-center justify-center mb-8 animate-pulse-slow">
            <span className="font-black text-[#172B36] text-2xl">A</span>
        </div>
        
        <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight">
          Start Building <span className="text-[#3CB3A5]">Now</span>
        </h1>
        <p className="text-[#F1F6F4]/60 max-w-lg mx-auto text-lg mb-10">
        ...
        {status === "success" ? (
          <div className="flex flex-col items-center animate-in fade-in zoom-in duration-500 mb-16">
            ...
            <h2 className="text-2xl font-bold text-[#F1F6F4] mb-2">You're on the list!</h2>
            <p className="text-[#F1F6F4]/60 text-center max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 w-full max-w-md mx-auto mb-16 relative">
            <input 
              ...
              className="flex-1 bg-[#F1F6F4]/5 border border-[#F1F6F4]/10 rounded-full px-6 py-4 text-[#F1F6F4] outline-none focus:border-[#FFC801] transition-colors disabled:opacity-50"
            />
            <button 
              type="submit" 
              disabled={status === "loading"}
              className="bg-[#FFC801] text-[#172B36] font-bold px-8 py-4 rounded-full hover:scale-105 transition-transform shadow-[0_0_20px_rgba(255,200,1,0.2)] disabled:opacity-50 disabled:hover:scale-100 min-w-[160px] flex justify-center items-center"
            >
              {status === "loading" ? (
                <div className="w-5 h-5 border-2 border-[#172B36]/20 border-t-black rounded-full animate-spin" />
              ) : (
                "Request Access"
              )}
            </button>
